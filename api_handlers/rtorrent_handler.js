...
async function makeXmlRpcRequest(serverConfig, methodName, params = []) {
    ...
    console.warn('rTorrent XML-RPC makeXmlRpcRequest is a basic implementation.');
    ...
    const paramsXml = params.map(p => {
        ...
        if (p && typeof p === 'object' && p.type === 'base64') { // Special handling for base64
            ...
            console.warn(`rTorrent handler: Parameter type ${typeof p} not fully supported, sending as string.`);
    ...
    try {
        ...
        if (responseText.includes("<fault>")) {
            console.error('rTorrent XML-RPC fault response:', responseText);
        ...
        console.warn("rTorrent: Could not parse specific success data from XML response:", responseText.substring(0, 500));
        return { success: true, data: responseText }; 

    } catch (error) {
        console.error('Error in rTorrent XML-RPC request or response parsing:', error);
        return { 
            success: false, 
            error: {
...
export async function addTorrent(torrentUrl, serverConfig, torrentOptions) {
    ...
    if (torrentOptions.downloadDir) {
        console.warn("rTorrent: downloadDir specified but setting it during initial load is not implemented in this basic handler. Requires post-add commands or complex load command.");
    ...
    if (torrentOptions.labels && torrentOptions.labels.length > 0) {
        console.warn("rTorrent: Labels/tags specified but setting them during initial load is not implemented. Requires post-add commands or complex load command.");
