...
async function makeRpcRequest(serverConfig, method, args = {}) {
    ...
    try {
        ...
        if (!response.ok) {
            return { success: false, error: { userMessage: `Vuze (XML WebUI) API request failed: ${response.status} ${response.statusText}` } };
        }

        const json = await response.json();
        if (json.result !== 'success') {
            return { success: false, error: { userMessage: `Vuze (XML WebUI) RPC Error: ${json.result}` } };
        }

        return { success: true, data: json.arguments };

    } catch (error) {
        debug.error('Error communicating with Vuze (XML WebUI):', error);
        return { success: false, error: { userMessage: `Could not contact Vuze (XML WebUI): ${error.message}` } };
    }
}
...
export async function testConnection(serverConfig) {
    ...
    if (result.success && result.data) {
        ...
        return {
            success: true,
            data: {
                message: 'Successfully connected to Vuze (XML WebUI).',
                version: version,
